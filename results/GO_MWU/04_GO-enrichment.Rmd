...
# To view TEs 
geneInfo %>% dplyr::select(SPID, protein_names) %>% unique() %>% filter(str_detect(protein_names, TE.genes)) %>% View()
...
jpeg(file = "../Figure7.jpeg", width = 550, height = 850, res = 150)
te.perc %>%
#  filter(module %in% modules) %>%
  ...
  geom_jitter(position=position_jitter(seed=4, width = 0.18), aes(fill=type, shape=type), size=6) +  #simplified figure
  ...
  theme(axis.title=element_text(size=13), axis.text=element_text(size=12), 
        legend.position="bottom", legend.text=element_text(size=10), legend.spacing.y = unit(.01, 'cm')) +
  ...
  guides(shape = guide_legend(nrow=2,byrow=TRUE, order=1, override.aes = list(size=4.25, fill=c("gray65", "gray30"))),
         size = guide_legend(order=0, override.aes = list(fill="white", shape=21))) +
