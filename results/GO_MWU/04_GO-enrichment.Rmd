...
filter(., grepl(GO,name)) %>% 
  ...
  select(seq, SPID, protein_names) %>% unique() %>% View()
...
# DAVID 
z <- c(degs,
       (geneInfo.mwu %>% filter(moduleColor == MOD) %>%
          filter(p.GS.pco2<0.05))$id)

read_delim(file = paste("DAVID_", MOD, "_DEGs-or-GS.txt", sep=""), delim = "\t") %>% 
  separate(Term, into = c("GO", "name"), sep = "~") %>% 
filter(., grepl("positive regulation of microglial cell activation",name)) %>%
  select(Genes) %>% noquote() %>% separate(Genes, sep = ",", into=c("a","b","c","d")) %>%
  pivot_longer(cols = everything()) %>%
  left_join(geneInfo.mwu %>% filter(moduleColor == MOD) %>% filter(id %in% z) %>% select(id, SPID),
            by=c("value"="SPID"))
...
te.perc %>% 
  filter(change == "Upregulated") %>% droplevels() %>%
  summarize(mean=mean(te.perc))

te.perc %>% 
  filter(change != "Upregulated") %>% filter(change != "Background") %>% droplevels() %>%
  summarize(mean=mean(te.perc))
...
te.perc %>%
  ...
  geom_text(position=position_jitter(width = .2,seed = 9), 
            aes(color=module, label=paste(100*round(te.perc, digits = 2), "%", sep=""))) + 
  theme_minimal() + 
  scale_colour_manual(name = "Module", values = sort(modules) %>% gsub("ivory", "wheat", .)) +
  ylab("Percent TEs") + xlab(NULL) + ggtitle("Percent of genes that are TEs by module") +
  geom_hline(yintercept = te.perc[te.perc$module=="background",]$te.perc, linetype="dashed", color="gray20")
```
...
# Upregulated Biological processes
up.go %>% filter(category %in% "GOTERM_BP_DIRECT") %>% group_by(gene_set, category, nseqs, Genes) %>%
summarise(pval=min(pval)) %>% distinct() %>% ungroup() %>% #for rows with duplicate grouping variabes, select one with lowest p-value
left_join(up.go %>% select(gene_set, term, ID, Genes, pval, p.adj, percent, Fold.Enrichment, OA.response)) %>% 
