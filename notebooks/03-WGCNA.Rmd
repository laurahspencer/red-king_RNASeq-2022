...
```
# If I want to load in filtered datasets also used for DESeq2 
```{r}
# load data
...
We choose a height cut of 0.25, corresponding to correlation of 0.75, to merge. (The WGCNA tutorial used a correlation threshold of 0.75, but I want to be a bit stricter when combining modules - so they have very similar patterns). 
...
annot = left_join(P.plat.bestblast, P.plat.blast, by=c("id"="geneID")) %>% 
...
genes.stats <- gene.counts.norm %>% group_by(Treatment, gene) %>% 
  ...
  ungroup() %>% 
  left_join(P.plat.bestblast %>% select(id, hit_acc, hit_desc, e_value), by=c("gene"="id")) %>%
...
```{r}
geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.brown, p.MM.brown) %>%
...
geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.cyan, p.MM.cyan) %>%
  filter(moduleColor %in% modules) %>%
...
geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.darkgreen, p.MM.darkgreen) %>%
  filter(moduleColor %in% modules) %>%
...
geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.green, p.MM.green) %>%
  filter(moduleColor %in% modules) %>%
...
geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.lightcyan1, p.MM.lightcyan1) %>%
  filter(moduleColor %in% modules) %>%
...
geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.pink, p.MM.pink) %>%
  filter(moduleColor %in% modules) %>%
...
geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.red, p.MM.red) %>%
  filter(moduleColor %in% modules) %>%
...
# Venn diagram of DEGs and significant genes in WGCNA pH-associated modules 
...
# When I filter for gens that are sign. associated with pH (p.GS.pH<0.05) AND module membership is sign. (p.MM<0.05) that drops to 72.2%
