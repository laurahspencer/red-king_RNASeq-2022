...
list.of.packages <- c("tidyverse", "RColorBrewer", "ggpmisc", "ggpubr", "ape", "pheatmap")
...
load(file = "../results/wgcna/dissTom") #load if needed 
```
...
module.lineplots[[i]] <-  ggplot(a, aes(x=pH, y=median)) + geom_line(aes(group=id), size=.01, color=colors[i]) +
...
pheatmap(counts.modules.matrix[,colnames(counts.modules.matrix) !="module"], 
         ...
         annotation_colors = list(Treatment=c(Ambient="#2c7bb6", Moderate="#fdae61", Low="#d7191c"),
                                  module=c(brown="brown", pink="pink", lightcyan1="lightcyan1", 
                                           red="red", cyan="cyan", green="green", darkgreen="darkgreen")),
...
IPA.wgcna <- geneInfo %>% 
  select(id,GS.pH, p.GS.pH,  
         ...
         p.MM.cyan, 
         p.MM.green, 
         p.MM.darkgreen) %>% remove_rownames()

save(IPA.wgcna, file = "../results/wgcna/IPA.wgcna")
load(file = "../results/IPA/IPA.degs") #IPA.degs

# Merge DEG stats and WGCNA stats, save to file for IPA 
left_join(IPA.degs, IPA.wgcna, by=c("geneID"="id")) %>%
  left_join(P.plat.bestblast %>% select(id, id_gtf), by=c("geneID"="id_gtf")) %>%
  select(id, everything(), -geneID) %>% 
write_delim(file = "../results/IPA/RKC-results-for-IPA.tab", delim = "\t",
