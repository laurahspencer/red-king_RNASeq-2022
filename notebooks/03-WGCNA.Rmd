...
#load(file = "../results/bowtie/per-gene-summary-stats") #genes.stats is object, but i create it here, so don't load (?!)
...
load(file = "../results/bowtie/counts.annot.Pcamt")
...
# ANSWER: 
# 92.8% of the DEGs were assigned to a pH-associated module. 
...
```{r}
# Write out separate tables for each module that contain two columns: 1) gene id, and 2) module membership, i.e. significance. For each module, that column contains MM for genes assigned to that module, and 0 for genes not assigned to that module. 
modules.interest <- c("lightsteelblue", "mediumpurple2", "coral2" )
