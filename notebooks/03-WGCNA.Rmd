...
genes.stats <- gene.counts.norm %>% group_by(Treatment, gene) %>% 
  ...
  ungroup() %>% 
  left_join(P.plat.bestblast %>% select(id, id_gtf, hit_acc, hit_desc, e_value), by=c("gene"="id_gtf")) %>%
...
#  select(id, moduleColor, GS.pH, p.GS.pH, !!as.symbol(modules.p[i]), !!as.symbol(modules.cor[i])) %>% 
  select(id, moduleColor, GS.pco2, p.GS.pco2, !!as.symbol(modules.p[i]), !!as.symbol(modules.cor[i])) %>% 
...
```{r}
# ggplotly(geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.brown, p.MM.brown) %>%
...
#   ggtitle("Brown"))
# 
# ggplotly(geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.cyan, p.MM.cyan) %>%
#   filter(moduleColor %in% modules) %>%
...
#   ggtitle("Cyan"))
# 
# ggplotly(geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.darkgreen, p.MM.darkgreen) %>%
#   filter(moduleColor %in% modules) %>%
...
#   ggtitle("Darkgreen"))
# 
# ggplotly(geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.green, p.MM.green) %>%
#   filter(moduleColor %in% modules) %>%
...
#   ggtitle("Green"))
# 
# ggplotly(geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.lightcyan1, p.MM.lightcyan1) %>%
#   filter(moduleColor %in% modules) %>%
...
#   ggtitle("Lightcyan1"))
# 
# ggplotly(geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.pink, p.MM.pink) %>%
#   filter(moduleColor %in% modules) %>%
...
#   ggtitle("Pink"))
# 
# ggplotly(geneInfo %>% select(id, hit_acc, hit_desc, moduleColor, GS.pH, p.GS.pH, MM.red, p.MM.red) %>%
#   filter(moduleColor %in% modules) %>%
...
geneInfo %>%
  ...
  filter(moduleColor %in% modules) %>% 
  filter(DEG=="DEG") %>% 
  ungroup() %>% select(n, sign) %>% colSums()/n.DEGs*100 
...
#  group_by(moduleColor, DEG) %>% summarise(n=n(), sign=sum(p.GS.pH<0.05), not.sign=sum(p.GS.pH>0.05)) %>% 
  group_by(moduleColor, DEG) %>% summarise(n=n(), sign=sum(p.GS.pco2<0.05), not.sign=sum(p.GS.pco2>0.05)) %>% 
  filter(DEG=="DEG") %>% 
  ungroup() %>% select(n, sign, not.sign) %>% colSums()/n.DEGs*100 
...
#  group_by(moduleColor) %>% summarise(n=n(), sign=sum(p.GS.pH<0.05), not.sign=sum(p.GS.pH>0.05)) %>% 
  ...
  select(moduleColor, n, sign, not.sign) %>% 
  mutate(perc.sig=sign/n) %>% 
  select(sign, not.sign) %>% colSums()
```
...
geneInfo %>%  
  filter(moduleColor %in% modules) %>%
  dplyr::count(moduleColor) #%>% select(n) %>% colSums()
...
IPA.input <- left_join(IPA.degs, IPA.wgcna, by=c("geneID"="id")) %>%
  left_join(P.plat.bestblast %>% select(id, id_gtf), by=c("geneID"="id_gtf")) %>%
  select(id, everything(), -geneID)

# Save to file for IPA 

IPA.input %>% select(id, colnames(IPA.degs[-1])) %>%
write_delim(file = "../results/IPA/RKC-for-IPA_DEGs.tab", delim = "\t",
              col_names = TRUE, quote = "none")

IPA.input %>% select(id, colnames(IPA.wgcna[-1])) %>%
...
#  select(id, moduleColor, GS.pH, p.GS.pH, !!as.symbol(modules.all.p[i]), !!as.symbol(modules.all.cor[i])) %>% 
  select(id, moduleColor, GS.pco2, p.GS.pco2, !!as.symbol(modules.all.p[i]), !!as.symbol(modules.all.cor[i])) %>% 
  left_join(genes.stats, by = c("id"="gene")) %>% 
  #group_by(Treatment, moduleColor) %>% mutate(mean_z = scale(mean)) %>% ungroup() %>% #use this line of code to z-transform 
  #filter(moduleColor %in% modules) %>% 
  mutate(pH=Treatment) %>%
