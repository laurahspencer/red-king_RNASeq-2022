...
for (i in 1:length(x)) {
pc.plots[[i]] <- ggplotly(ggplot(tab.expr.annot, 
                      aes(x=!!as.symbol(pc.axes[x[i]]),y=!!as.symbol(pc.axes[y[i]]))) + 
  geom_point(aes(col=Treatment, fill=Tank), size=3.5, alpha=0.85, pch=20) + 
...
res.pco2.ML %>% as.data.frame() %>% select(log2FoldChange, padj) %>% rename_with( ~ paste0("Mod_vs_Low_", .x))) %>% 
  ...
  write_delim(file = "../results/deseq2/DESeq2-results-for-IPA.tab", delim = "\t", 
