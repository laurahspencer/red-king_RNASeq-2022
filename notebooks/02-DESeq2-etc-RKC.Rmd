...
```{r}
# IMPORTANT NOTE: to use data for ALL genes, use object "counts.t". To remove genes with mean count less than ten across all samples, use object "counts.ts" 
...
genes.stats <- gene.counts.norm %>% group_by(Treatment, gene) %>% 
  ...
  left_join(P.plat.blast.GO %>% select(geneID, SPID, `Protein names`, evalue), by=c("gene"="geneID"))
...
# What's the range of CV of the normalized counts, what is considered low-level variation? Q1 extends to 0.04
...
genes.stats %>% filter(mean>10 & cv<.04 & change=="Up-regulated" & Treatment!="Ambient") %>% View()

# Which genes were highly DOWN-regulated (mean of transformed counts >10) consistently within an OA treatment? (CV<0.04)
genes.stats %>% filter(mean>10 & cv<.04 & change=="Down-regulated" & Treatment!="Ambient") %>% View()
...
a <-  plotCounts(dds.DESeq.pH, gene="evm.TU.HiC_scaffold_16.208", intgroup=c("Treatment"), returnData = TRUE) %>% 
  mutate(Treatment=factor(Treatment, levels=c("Ambient", "Moderate", "Low")))
...
ggplot(a %>% rownames_to_column("sample"),
  ...
  geom_point(size=2, position=position_jitter(w = 0.15,h = 0)) +
    #geom_text(size=2) +
    theme_bw() +
    #ggtitle("superoxide dismutase [Mn], mitochondrial-like\nevm.TU.HiC_scaffold_64.12") +
...
# Q1 = 0.03, use that 
f %>% filter(Treatment!="Ambient") %>% filter(DEG=="DEG") %>% filter(change=="Up-regulated") %>% filter(cv<=0.03) %>% View()
...
f %>% filter(Treatment!="Ambient") %>% filter(DEG=="DEG") %>% filter(change=="Up-regulated") %>% filter(cv<=0.03) %>% 
...
a <-  plotCounts(dds.DESeq.pH, gene="evm.TU.HiC_scaffold_33.177", intgroup=c("Treatment"), returnData = TRUE) %>% 
  mutate(Treatment=factor(Treatment, levels=c("Ambient", "Moderate", "Low")))
