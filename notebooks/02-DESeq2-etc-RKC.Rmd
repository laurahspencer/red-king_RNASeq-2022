...
```{r}
# DEGs between Ambient & Moderate (includes 733 annotated genes)
...
# DEGs between Ambient & Low (includes 967 annotated genes)
diffex.AL %>% as.data.frame() %>% rownames_to_column("gene") %>% dplyr::select(gene, padj) %>% 
  left_join(P.plat.blast.GO %>% dplyr::select(geneID, SPID, `Protein names`), by = c("gene"="geneID")) %>% 
  dplyr::select(SPID) %>%  na.omit() %>% unlist() %>% as.vector() %>% write_clip()


# DEGs between Moderate & Low (includes just 2 annotated genes) <----- Neither of these were registered by DAVID 
