...
genes.stats <- gene.counts.norm %>% group_by(Treatment, gene) %>% 
  ...
  left_join(cbind(res.pco2.AM %>% as.data.frame() %>% rename("log2FoldChange"="L2FC.AM", "padj"="padj.AM") %>% 
  rownames_to_column("gene") %>% dplyr::select(gene, L2FC.AM, padj.AM),
res.pco2.AL %>% as.data.frame() %>% rename("log2FoldChange"="L2FC.AL", "padj"="padj.AL") %>% 
  rownames_to_column("gene") %>% dplyr::select(L2FC.AL, padj.AL),
res.pco2.ML %>% as.data.frame() %>% rename("log2FoldChange"="L2FC.ML", "padj"="padj.ML") %>% 
