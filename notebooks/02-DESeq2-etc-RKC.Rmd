...
```{r}
# dds.multi.DESeq <- dds.DESeq #duplicate DESeq object for multi-factor design 
...
```{r}
dds.pH.df <- as.data.frame(colData(dds.pH)[,c("Treatment"), drop=FALSE])



diffex.AM <- subset(res.pco2.AM, padj < 0.05)
diffex.AM.counts <- subset(counts(dds.DESeq.pH), rownames(dds.DESeq.pH) %in% rownames(diffex.AM))

dds.pH.AM <- dds.pH.df[match(colnames(diffex.AM.counts), rownames(dds.pH.df)),]
all(colnames(diffex.AM.counts) == rownames(dds.pH.df)) #double check that samples are still in same order 

pheatmap(diffex.AM.counts[rownames(diffex.AM[order(diffex.AM$padj),]),], cluster_rows=F, 
         ...
         annotation_col=dds.pH.df, color=c("dodgerblue3", "goldenrod1"))

pheatmap(diffex.counts, cluster_rows=T, show_rownames=F, cluster_columns=T, na.rm=TRUE, 
         scale="row", main = "(Adults) DEGs by direct pCO2 exposure, clustered", fontsize_col = 7, fontsize_row = 7, 
         annotation_col=dds.df, color=c("dodgerblue3", "goldenrod1"))
```
