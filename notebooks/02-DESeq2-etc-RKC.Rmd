...
```
### % of DEGs that were downregulated in OA - here we only consider 7.8 and 7.6 vs. ambient (8.0)
...
# Upregulated DEGs in Moderate (compared to ambient) that are also expressed very consistently 
res.pco2.AL %>% as.data.frame() %>% rownames_to_column("gene") %>% 
  filter(gene %in% c(unique(rownames(diffex.AM, diffex.AL)))) %>% 
...
## No significantly enriched processes in those "critical" genes according to GO_MWU. Instead, I used Go_MWU to look at all GO categories (see GO_MWU script). 
