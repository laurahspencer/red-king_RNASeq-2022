...
res.pco2.AM %>% as.data.frame() %>% rownames_to_column("gene") %>% filter(padj<0.05) %>% filter(log2FoldChange>0) %>% 
  left_join(P.camt.blast.GO %>% dplyr::select(gene_id, gene_ontology_i_ds, spid), by = c("gene"="gene_id")) %>% 
  dplyr::select(spid) %>%  na.omit() %>% unlist() %>% as.vector() %>% write_clip()
...
res.pco2.AL %>% as.data.frame() %>% rownames_to_column("gene") %>% filter(padj<0.05) %>% filter(log2FoldChange>0) %>% 
  left_join(P.camt.blast.GO %>% dplyr::select(gene_id, gene_ontology_i_ds, spid), by = c("gene"="gene_id")) %>% 
  dplyr::select(spid) %>%  na.omit() %>% unlist() %>% as.vector() %>% write_clip()
...
res.pco2.ML %>% as.data.frame() %>% rownames_to_column("gene") %>% filter(padj<0.05) %>% filter(log2FoldChange>0) %>% 
  left_join(P.camt.blast.GO %>% dplyr::select(gene_id, gene_ontology_i_ds, spid), by = c("gene"="gene_id")) %>% 
  dplyr::select(spid) %>%  na.omit() %>% unlist() %>% as.vector() %>% write_clip()
...
pdf(file="../results/Figure5-downregulated.pdf", height = 5.35, width = 6)
...
pdf(file="../results/Figure5-upregulated.pdf", height =3, width = 5.5)
...
ggplot(aes(y = process, x=contrast, color=up.treatment)) + 
 ...
 facet_wrap(~category,scales="free", nrow = 2, 
  ...
  annotate("text", x = 2, y = -.35, label = "Severe OA\n(pH 7.3)", size=2.5, col="gray15") +
#  annotate("text", x = 3, y = -.45, label = "Moderate OA\nvs. Severe OA", size=2, col="gray15") +
coord_cartesian(clip = "off", ylim = c(-.7, 13.5))
# oord_cartesian(clip = "off", ylim = c(-0.6, 20))
dev.off()
```