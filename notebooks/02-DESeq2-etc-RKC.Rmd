...
fragments <- data.frame(rowSums(counts.ts)) %>% 
                  dplyr::rename(read.total = 1) %>% 
                  rownames_to_column(var="sample") %>%
  left_join(sample.info %>% select(Sample, Treatment, Tank), by=c("sample"="Sample"))
...
Let’s conduct perMANOVA to test for differences in fish species composition between sites in different major river basins. 

```{r}
# Effect of treatment 
vsd.t <- t(assay(vsd.pH)) %>% as.data.frame()
print(perm <- adonis(vsd.t ~ Treatment, data=sample.info %>% remove_rownames() %>% column_to_rownames("Sample"), permutations = 1000, method="bray"))

# Pairwise comparisons 
vsd.t <- sample.info %>% select(Sample, Treatment, Tank) %>% left_join(t(assay(vsd.pH)) %>% as.data.frame() %>% rownames_to_column("Sample"))
pairwise.adonis(vsd.t[,-1:-3], vsd.t$Treatment)
```
...
## For DEGs - Reaction Norm Plot  - REDO USING PCO2! 
